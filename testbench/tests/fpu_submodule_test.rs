use riscv_core::{
    create_fpu_adder_runtime, create_fpu_classifier_runtime, create_fpu_comparator_runtime,
    create_fpu_float_to_int_runtime, create_fpu_int_to_float_runtime,
    create_fpu_multiplier_runtime, create_fpu_sqrt_runtime, FpuAdder, FpuClassifier, FpuComparator,
    FpuFloatToInt, FpuIntToFloat, FpuMultiplier, FpuSqrt,
};
...
#[test]
fn test_fpu_adder_basic() {
    let runtime = create_fpu_adder_runtime().expect("Failed to create adder runtime");
    let mut dut = runtime.create_model_simple::<FpuAdder>().unwrap();
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, THREE, "1.0 + 2.0 should equal 3.0");
}

#[test]
fn test_fpu_adder_zero() {
    let runtime = create_fpu_adder_runtime().expect("Failed to create adder runtime");
    let mut dut = runtime.create_model_simple::<FpuAdder>().unwrap();
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, ONE, "1.0 + 0.0 should equal 1.0");
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, TWO, "0.0 + 2.0 should equal 2.0");
}

#[test]
fn test_fpu_adder_negative() {
    let runtime = create_fpu_adder_runtime().expect("Failed to create adder runtime");
    let mut dut = runtime.create_model_simple::<FpuAdder>().unwrap();
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, POS_ZERO, "1.0 + (-1.0) should equal 0.0");
}

#[test]
fn test_fpu_adder_subtraction() {
    let runtime = create_fpu_adder_runtime().expect("Failed to create adder runtime");
    let mut dut = runtime.create_model_simple::<FpuAdder>().unwrap();
    ...
    dut.is_sub = 1;
    dut.eval();
    assert_eq!(dut.result, TWO, "3.0 - 1.0 should equal 2.0");
...
fn test_fpu_adder_infinity() {
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, POS_INF, "1.0 + Inf should equal Inf");
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, POS_INF, "Inf + Inf should equal Inf");

    // Test: Inf - Inf = NaN (invalid operation)
    dut.a = POS_INF;
    dut.b = POS_INF;
    dut.is_sub = 1;
    dut.eval();
    assert_eq!(dut.result, QNAN, "Inf - Inf should equal NaN");
}

#[test]
fn test_fpu_adder_nan() {
    let runtime = create_fpu_adder_runtime().expect("Failed to create adder runtime");
    let mut dut = runtime.create_model_simple::<FpuAdder>().unwrap();
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, QNAN, "NaN + 1.0 should equal NaN");
    ...
    dut.is_sub = 0;
    dut.eval();
    assert_eq!(dut.result, QNAN, "1.0 + NaN should equal NaN");
}

// ========== FPU Multiplier Tests ==========

#[test]
fn test_fpu_multiplier_basic() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();

    // Test: 2.0 * 2.0 = 4.0
    dut.a = TWO;
    dut.b = TWO;
    dut.eval();
    assert_eq!(dut.result, FOUR, "2.0 * 2.0 should equal 4.0");

    // Test: 2.0 * 3.0 = 6.0
    dut.a = TWO;
    dut.b = THREE;
    dut.eval();
    assert_eq!(dut.result, SIX, "2.0 * 3.0 should equal 6.0");
}

#[test]
fn test_fpu_multiplier_one() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();
    ...
    dut.b = ONE;
    dut.eval();
    assert_eq!(dut.result, THREE, "3.0 * 1.0 should equal 3.0");
}

#[test]
fn test_fpu_multiplier_zero() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();
    ...
    dut.b = POS_ZERO;
    dut.eval();
    assert_eq!(dut.result, POS_ZERO, "3.0 * 0.0 should equal 0.0");

    // Test: 0.0 * 0.0 = 0.0
    dut.a = POS_ZERO;
    dut.b = POS_ZERO;
    dut.eval();
    assert_eq!(dut.result, POS_ZERO, "0.0 * 0.0 should equal 0.0");
}

#[test]
fn test_fpu_multiplier_negative() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();
    ...
    dut.b = NEG_ONE;
    dut.eval();
    assert_eq!(dut.result, NEG_TWO, "2.0 * -1.0 should equal -2.0");

    // Test: -1.0 * -1.0 = 1.0
    dut.a = NEG_ONE;
    dut.b = NEG_ONE;
    dut.eval();
    assert_eq!(dut.result, ONE, "-1.0 * -1.0 should equal 1.0");
}

#[test]
fn test_fpu_multiplier_infinity() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();
    ...
    dut.b = POS_INF;
    dut.eval();
    assert_eq!(dut.result, POS_INF, "2.0 * Inf should equal Inf");

    // Test: Inf * 0.0 = NaN (invalid operation)
    dut.a = POS_INF;
    dut.b = POS_ZERO;
    dut.eval();
    assert_eq!(dut.result, QNAN, "Inf * 0.0 should equal NaN");

    // Test: 0.0 * Inf = NaN
    dut.a = POS_ZERO;
    dut.b = POS_INF;
    dut.eval();
    assert_eq!(dut.result, QNAN, "0.0 * Inf should equal NaN");
}

#[test]
fn test_fpu_multiplier_nan() {
    let runtime = create_fpu_multiplier_runtime().expect("Failed to create multiplier runtime");
    let mut dut = runtime.create_model_simple::<FpuMultiplier>().unwrap();
