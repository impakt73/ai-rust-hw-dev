...
use testbench::{
    with_fpu_classifier_model, with_fpu_comparator_model, with_fpu_float_to_int_model,
    with_fpu_int_to_float_model, with_fpu_sqrt_model,
};
...
fn test_fpu_classifier_nan() {
    with_fpu_classifier_model(|mut dut| {
        // Test Quiet NaN
        dut.val = QNAN;
        dut.eval();
...
fn test_fpu_comparator_basic() {
    with_fpu_comparator_model(|mut dut| {
        // Test: 1.0 < 2.0 -> true
        dut.a = ONE;
        dut.b = TWO;
        dut.eval();
...
fn test_fpu_int_to_float_signed_positive() {
    with_fpu_int_to_float_model(|mut dut| {
...
fn test_fpu_float_to_int_signed_positive() {
    with_fpu_float_to_int_model(|mut dut| {
