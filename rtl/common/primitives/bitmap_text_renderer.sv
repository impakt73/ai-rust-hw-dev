...
module bitmap_text_renderer #(
    ...
    // video_sync resets its scan counters into the horizontal blanking region
    // (start of the front porch on the last line).  After reset deasserts, the
    // scanner walks through the remaining blanking interval before the first
    // active pixel.  The front-porch budget above ensures there are at least
    // FONT_PIPELINE_CYCLES of blanking during which the prefetch pipeline fills
    // with valid data, so no special reset seeding is needed here.
