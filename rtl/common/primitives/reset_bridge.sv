...
//
// Parameters:
//   STAGES - Number of synchronizer stages used to release reset (default: 2)
//
// Interface:
...
module reset_bridge #(
    ...
    logic reset_sync_regs [0:STAGES-1] = '{default: 1'b1};

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            for (int i = 0; i < STAGES; i++) begin
                reset_sync_regs[i] <= 1'b1;
            end
        end else begin
            reset_sync_regs[0] <= 1'b0;
            for (int i = 1; i < STAGES; i++) begin
                reset_sync_regs[i] <= reset_sync_regs[i-1];
            end
        end
    end

    assign rst = reset_sync_regs[STAGES-1];
