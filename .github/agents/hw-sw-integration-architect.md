...
- When a datapath payload register is protected by a separate `valid`, `pending`, or equivalent control flag, reset the control flag but not the payload register itself; initialize the payload when the flag is asserted and require consumers to ignore the payload while the flag is low
