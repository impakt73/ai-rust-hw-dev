...
- When a datapath payload register has a separate `valid`, `pending`, or similar control bit, reset the control bit rather than the payload register itself. The payload should be written when the control bit goes high, and downstream logic must ignore the payload whenever that control bit is low.
