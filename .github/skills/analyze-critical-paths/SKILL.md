...
- **`ice40_alchitry_cu`**
  - Critical paths: prefer `build/ice40_alchitry_cu/riscv_fpga_timing.rpt`; fall back to the final detailed timing section in `build/ice40_alchitry_cu/nextpnr.log` when `--report` is unavailable
  - Cross-check stats: `build/ice40_alchitry_cu/riscv_fpga_stats.json`
- **`ecp5_icepi_zero`**
  - Critical paths: prefer `build/ecp5_icepi_zero/riscv_fpga_timing.rpt`; fall back to the final detailed timing section in `build/ecp5_icepi_zero/nextpnr.log` when `--report` is unavailable
...
For `ice40_alchitry_cu` and `ecp5_icepi_zero`, prefer the dedicated `riscv_fpga_timing.rpt` generated by nextpnr's `--report` flag. If that feature is unavailable in the local nextpnr build, use the final routed timing section near the end of `nextpnr.log`.

- Do **not** use earlier placement-only estimates as the top-path ranking
- Do **not** rely on intermediate summaries when the dedicated routed timing report or final routed log section is present
- Do **not** treat old timing-analysis docs as evidence without re-reading the fresh routed timing source
