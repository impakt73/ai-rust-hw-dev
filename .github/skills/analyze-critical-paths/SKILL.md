...
### Important Rule for ECP5

For `ecp5_icepi_zero`, prefer the dedicated `riscv_fpga_timing.rpt` generated by nextpnr's `--report` flag. If that feature is unavailable in the local nextpnr build, use the final routed timing section near the end of `nextpnr.log`.
