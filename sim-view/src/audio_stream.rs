...
impl AudioStream {
    ...
    fn create_stream(
        ...
        let (stream_config, sample_format) = if let Some(cfg) = config {
            // Build custom StreamConfig from AudioConfig
            ...
            // Try to find a supported format matching our requirements
            // For simplicity, we'll use the device's preferred format
            let supported_config = device
                .default_output_config()
                .map_err(|e| format!("Failed to get default audio config: {}", e))?;

            log::info!(
                "Requested: {} Hz, {} channels | Device default: {} Hz, {} channels, format: {:?}",
