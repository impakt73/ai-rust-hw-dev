...
fn test_device_registration_success() {
    ...
    // Register device at 0x9000_0000
    let dev = Box::new(MockDevice::new(256, "TestDevice"));
    let result = bus.register_device(0x9000_0000, dev);
    assert!(result.is_ok());
...
fn test_device_registration_overlap_error() {
    ...
    // Register first device at 0x9000_0000, size 256 bytes (0x9000_0000 - 0x9000_0100)
    let dev1 = Box::new(MockDevice::new(256, "Device1"));
    bus.register_device(0x9000_0000, dev1).unwrap();

    // Try to register overlapping device at 0x9000_0080 (overlaps with Device1)
    let dev2 = Box::new(MockDevice::new(256, "Device2"));
    let result = bus.register_device(0x9000_0080, dev2);
    ...
    {
        assert_eq!(new_base, 0x9000_0080);
        assert_eq!(new_end, 0x9000_0180);
        assert_eq!(existing_base, 0x9000_0000);
        assert_eq!(existing_end, 0x9000_0100);
...
#[test]
fn test_device_registration_upper_half_mmio_allowed() {
    let mut bus = SystemBus::new();

    // Rust MMIO devices can be carved out from upper-half addresses.
    let dev = Box::new(MockDevice::new(256, "TestDevice"));
    let result = bus.register_device(0x8100_0000, dev);
    assert!(result.is_ok());
}
...
fn test_device_registration_adjacent_devices_ok() {
    ...
    // Register first device at 0x9000_0000, size 256 bytes
    let dev1 = Box::new(MockDevice::new(256, "Device1"));
    bus.register_device(0x9000_0000, dev1).unwrap();

    // Register adjacent device at 0x9000_0100 (immediately after Device1)
    let dev2 = Box::new(MockDevice::new(256, "Device2"));
    let result = bus.register_device(0x9000_0100, dev2);
    assert!(result.is_ok());
}
...
fn test_device_registration_invalid_base_alignment() {
    ...
    let dev = Box::new(MockDevice::new(256, "TestDevice"));
    let result = bus.register_device(0x9000_0001, dev);

    assert!(matches!(
        result,
        Err(RegistrationError::InvalidBaseAlignment {
            base_addr: 0x9000_0001
        })
    ));
}
...
fn test_device_read_write_routing() {
    ...
    // Register device at 0x9000_0000
    let dev = Box::new(MockDevice::new(256, "TestDevice"));
    bus.register_device(0x9000_0000, dev).unwrap();

    // Write a value to the device
    bus.write_word(0x9000_0000, 0x12345678);

    // Read it back
    let value = bus.read_word(0x9000_0000);
    assert_eq!(value, 0x12345678);

    // Write to different offset
    bus.write_word(0x9000_0004, 0xABCDEF00);
    let value = bus.read_word(0x9000_0004);
...
fn test_multiple_devices_independent() {
    ...
    let dev2 = Box::new(MockDevice::new(256, "Device2"));
    bus.register_device(0x9000_0100, dev2).unwrap();

    // Write different values to each device
    bus.write_word(0x9000_0000, 0x11111111);
    bus.write_word(0x9000_0100, 0x22222222);

    // Verify each device has its own value
    assert_eq!(bus.read_word(0x9000_0000), 0x11111111);
    assert_eq!(bus.read_word(0x9000_0100), 0x22222222);
}
...
fn test_reset_all_devices_called() {
    ...
    assert_eq!(bus.read_word(0x9000_0000), 1);

    // Call reset again
    bus.reset_all_devices();

    // reset_count should now be 2
    assert_eq!(bus.read_word(0x9000_0000), 2);
}
...
fn test_clock_cycle_all_devices_called() {
    ...
    assert_eq!(bus.read_word(0x9000_0004), 11);
}
...
fn test_reset_clears_device_state() {
    ...
    bus.write_word(0x9000_0008, 0x12345678);
    bus.write_word(0x9000_000C, 0xABCDEF00);

    // Verify values are written
    assert_eq!(bus.read_word(0x9000_0008), 0x12345678);
    assert_eq!(bus.read_word(0x9000_000C), 0xABCDEF00);

    // Call reset (which should clear registers in StatefulMockDevice)
    bus.reset_all_devices();

    // Verify registers are cleared
    assert_eq!(bus.read_word(0x9000_0008), 0);
    assert_eq!(bus.read_word(0x9000_000C), 0);
...
fn test_multiple_stateful_devices_independent() {
    ...
    assert_eq!(bus.read_word(0x9000_0004), 5);
    assert_eq!(bus.read_word(0x9000_0104), 5);
    ...
    assert_eq!(bus.read_word(0x9000_0000), 1);
    assert_eq!(bus.read_word(0x9000_0100), 1);
...
fn test_device_with_default_reset_and_clock_cycle() {
    ...
    assert_eq!(bus.read_word(0x9000_0000), 0x12345678);
