...
/// Response from a host-initiated bus request (re-exported from host-bus-handler)
pub type HostBusResponse = BusResponse;
...
impl<'a> SimulatorView<'a> {
    ...
    pub fn send_bus_request(
        ...
        // Create the request and send via handler
        let request = if we {
        ...
        self.host_bus_handler
            ...
            .map_err(|e| format!("Failed to send bus request: {:?}", e))
    }
    ...
    T: FnMut(&InstructionTrace),
{
    ...
    pub fn new(
        ...
        // Note: Memory latency simulation is now handled in RTL
    ...
    /// Handle the host bus interface protocol using HostBusHandler
    ///
    /// This method uses the HostBusHandler crate to manage the bidirectional
    /// serialized bus protocol between host and FPGA.
    fn handle_host_bus_interface(&mut self) {
        // Handle RX bytes from FPGA (host_tx_* signals)
        if self.cpu.host_tx_valid == 1 && self.host_bus_handler.can_accept_rx() {
            // Transfer byte to handler, expect it to succeed since we checked can_accept_rx
            self.host_bus_handler
                .transfer_rx_byte(self.cpu.host_tx_data)
                .expect("Host bus handler should accept RX byte");
        }

        // Set host_tx_ready based on whether handler can accept more data
        self.cpu.host_tx_ready = if self.host_bus_handler.can_accept_rx() {
            1
        } else {
            0
        };

        // Handle TX bytes to FPGA (host_rx_* signals)
        ...
        // Process any incoming requests from CPU
        if let Ok(request) = self.host_bus_handler.accept_request() {
            // Perform the bus operation
            let response = if request.we {
                // Write operation
                match request.size {
                    AccessSize::Byte => self.bus.write_byte(request.addr, request.wdata as u8),
                    AccessSize::Halfword => {
                        self.bus.write_halfword(request.addr, request.wdata as u16)
                    }
                    AccessSize::Word => self.bus.write_word(request.addr, request.wdata),
                }
                BusResponse::write_ack(request.size)
            } else {
                // Read operation
                let rdata = match request.size {
                    AccessSize::Byte => self.bus.read_byte(request.addr) as u32,
                    AccessSize::Halfword => self.bus.read_halfword(request.addr) as u32,
                    AccessSize::Word => self.bus.read_word(request.addr),
                };
            ...
            self.host_bus_handler
                ...
                .expect("Should be able to complete request");
        }
    }
    ...
    pub fn reset(&mut self, boot_pc: u32) -> Result<(), HungStateError> {
        ...
        self.cpu.host_tx_ready = 1; // Ready to receive after reset
        self.cpu.host_rx_valid = 0;
