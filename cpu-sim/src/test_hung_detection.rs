...
) -> Result<Simulator<'static, fn(u32), fn(&riscv_core::trace::InstructionTrace)>, String> {
...
fn test_infinite_loop_detection() {
    ...
    let instructions = vec![infinite_loop_instr.to_le_bytes()];
    ...
    if let Err(panic_info) = result {
        if let Some(msg) = panic_info.downcast_ref::<String>() {
            assert!(
                msg.contains("PC stuck"),
                "Expected PC stuck error, got: {}",
                msg
            );
...
fn test_valid_pc_range_auto_set_from_elf() {
    ...
    assert!(
        result.is_ok(),
        "Should successfully load and run ELF with auto-detected PC range"
    );
