...
fn main() -> ! {
    unsafe {
        ...
        write_volatile(FIFO_DATA as *mut u32, 0x11111111);  // Marker
        for i in 0..4 {
            let byte = core::ptr::read(ptr.add(i));
            write_volatile(FIFO_DATA as *mut u32, byte as u32);
        }
        write_volatile(FIFO_DATA as *mut u32, 0x22222222);  // Marker
        
        // Write a single byte at offset 1
        core::ptr::write(ptr.add(1), 0xBBu8);
        
        // Read back all 4 bytes
        write_volatile(FIFO_DATA as *mut u32, 0x33333333);  // Marker
        for i in 0..4 {
            let byte = core::ptr::read(ptr.add(i));
            write_volatile(FIFO_DATA as *mut u32, byte as u32);
        }
        write_volatile(FIFO_DATA as *mut u32, 0x44444444);  // Marker
    }
