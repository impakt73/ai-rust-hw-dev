...
/// Write a stereo sample to the ring buffer (silence = 0 values)
/// For stereo: writes 4 bytes (2 × i16, one for left and one for right), all zeros
fn write_stereo_silence(buffer_base: u32, offset: u32) {
    unsafe {
        let addr = buffer_base + offset;
        // Write 4 zero bytes for stereo silence
...
/// Fill the audio buffer with silence (zero samples)
/// Fills as much of the available space as possible
fn fill_audio_buffer_with_silence(write_ptr: &mut u32) -> u32 {
    ...
    // Write silence samples to fill the available space
    // This is much faster than generating sine waves
    for _ in 0..samples_to_write {
        write_stereo_silence(RING_BUFFER_BASE, *write_ptr);
...
fn main() -> ! {
    unsafe {
        ...
        // Main infinite loop - generate silence as fast as possible
        loop {
            // Check the status of the circular audio buffer pointers
            // and fill any available space in the buffer with silence samples
            fill_audio_buffer_with_silence(&mut audio_write_ptr);
        }
    }
}
