...
fn test_decompress_c_addi4spn_illegal() {
    ...
    assert_eq!(dut.is_compressed, 1);
    assert_eq!(dut.is_valid, 0, "C.ADDI4SPN with nzuimm=0 should be illegal");
}
...
fn test_decompress_c_j() {
    ...
    println!("C.J test - insn_16 = 0x{:04x} = 0b{:016b}", insn_16, insn_16);
    println!("C.J test - insn_32 = 0x{:08x}", dut.insn_32);
    println!("C.J test - opcode = 0x{:02x} (expected 0x6F)", dut.insn_32 & 0x7F);
