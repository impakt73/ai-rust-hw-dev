...
fn test_audio_pattern() {
    ...
    let configs_for_callback = captured_configs.clone();
    ...
    // Register Audio device at AUDIO_BASE at runtime creation
    ...
    let startup_status = read_word_with_timeout(runtime.as_mut(), AUDIO_BASE + 0x08, SHORT_TIMEOUT);
    assert_ne!(
        startup_status & (1 << 0),
        0,
        "DMA_READY should be set before boot"
    );
    ...
    let final_status = read_word_with_timeout(runtime.as_mut(), AUDIO_BASE + 0x08, SHORT_TIMEOUT);
    assert_ne!(
        final_status & (1 << 0),
        0,
        "DMA_READY should be set after completion"
    );

    println!("\n=== Audio Pattern Test Results ===");
    println!("Test program completed successfully");

    let sample_batches = captured_samples.lock().expect("samples lock poisoned");
    let configs = captured_configs.lock().expect("configs lock poisoned");
