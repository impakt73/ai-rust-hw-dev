...
//!
//! This file contains portable RTL verification tests that can run by loading instruction
//! programs, waiting for CPU halt, and validating behavior by reading memory via host bus
//! requests after halt.
...
fn test_cpu_byte_halfword_mixed() {
    ...
    assert_eq!(
        read_word_with_timeout(runtime.as_mut(), DRAM_BASE + 0x10, SHORT_TIMEOUT),
        0xFFFF_FF80
    );
    assert_eq!(
        read_word_with_timeout(runtime.as_mut(), DRAM_BASE + 0x14, SHORT_TIMEOUT),
        0x0000_0080
    );
    assert_eq!(
        read_word_with_timeout(runtime.as_mut(), DRAM_BASE + 0x18, SHORT_TIMEOUT),
        0xFFFF_FFFF
    );
    assert_eq!(
        read_word_with_timeout(runtime.as_mut(), DRAM_BASE + 0x1C, SHORT_TIMEOUT),
        0x0000_FFFF
    );
}
...
fn test_cpu_ecall_instruction() {
    ...
    let mut instructions = vec![addi(1, 0, 42), lui(9, DRAM_BASE), sw(9, 1, 0)];
    instructions.extend(tohost_termination(7, 8, SUCCESS_CODE));
...
fn test_cpu_ebreak_instruction() {
    ...
    let mut instructions = vec![addi(1, 0, 100), lui(9, DRAM_BASE), sw(9, 1, 0)];
    instructions.extend(tohost_termination(7, 8, SUCCESS_CODE));
