...
10. [FPGA Considerations](#fpga-considerations)
...
**Single-Cycle Critical Path:** ~65 ns = ~15 MHz max frequency

**Multi-Cycle Critical Path:** ~35 ns = ~28 MHz max frequency

**Expected Improvement:** ~1.9x frequency improvement (15 MHz → 28 MHz)

**Further Optimizations:**
- Multi-cycle multiply/divide: Break 64-bit multiply into 2-3 cycles (50+ MHz)
- State machine encoding: One-hot vs. binary
- Memory pipelining: Add memory wait states if needed

### Resource Utilization

**Expected Changes:**

| Resource | Single-Cycle | Multi-Cycle | Change |
|----------|--------------|-------------|--------|
| LUTs (logic) | ~2000 | ~2500 | +25% |
| Flip-Flops (registers) | ~100 | ~600 | +500% |
| Block RAMs | 0 | 0 | 0% |
| DSP Slices (multiply) | ~4 | ~4 | 0% |

**Analysis:**
- LUTs increase due to state machine and control logic
- FFs increase significantly due to pipeline registers (expected and desired)
- More registers → better timing, easier to meet constraints
- Trade-off: More area for better frequency

### Target FPGA Platforms

**Recommended Devices:**
- **Xilinx Artix-7** (e.g., Arty A7-35T)
- **Intel Cyclone V** (e.g., Cyclone V GX)
- **Lattice ECP5** (e.g., LFE5U-25F)
...
**Instruction Mix (Typical RISC-V Program):**
- R-type: 30%
- I-type ALU: 20%
- Load: 20%
- Store: 10%
...
- Total cycles for 100 instructions: **100 cycles**

**Multi-Cycle Performance:**
- R-type (30): 4 cycles × 30 = 120
- I-type (20): 4 cycles × 20 = 80
- Load (20): 5 cycles × 20 = 100
- Store (10): 4 cycles × 10 = 40
- Branch (15): 3 cycles × 15 = 45
- Jump (3): 4 cycles × 3 = 12
- Other (2): 3 cycles × 2 = 6
- **Total cycles for 100 instructions: ~403 cycles**

**CPI (Cycles Per Instruction):**
- Single-cycle: 1.0 CPI
- Multi-cycle: 4.03 CPI

**Performance Comparison:**
- Multi-cycle is ~4x slower in cycles
- But runs at ~1.9x higher frequency
- **Net performance: ~2.1x slower** than single-cycle

**MIPS Comparison (at max frequency):**
- Single-cycle @ 15 MHz: 15 MIPS
- Multi-cycle @ 28 MHz: ~7 MIPS (28 MHz / 4.03 CPI)

**Trade-Off:** Accept ~2x slowdown for FPGA synthesis capability

### Future Optimizations

**Reduce Cycle Count:**
1. State Merging: Combine DECODE + EXECUTE for branches (2 cycles total)
2. Variable Cycle Count: Fast-path for ALU-only instructions (3 cycles)
3. Multi-cycle divide: Iterative division (reduces combinational depth)

**Optimized CPI: ~3.2** → **~8.8 MIPS @ 28 MHz**
