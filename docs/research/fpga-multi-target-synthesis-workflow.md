...
The critical path identified in `docs/fpga/SYNTHESIS_ANALYSIS.md`—through the 32-bit ALU carry chain then `host_bus_mux` and `registered_bus.sv`—will be significantly shorter on ECP5, Artix-7, and Cyclone V due to faster carry chains and better routing resources.
